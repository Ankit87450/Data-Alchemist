...
// This new component encapsulates the API call and its loading state.
function AIFixButton({ entity, rowIndex, columnId, cellValue, errorContext }) {
  const { dispatch } = useAppContext();
  const [isLoading, setIsLoading] = useState(false);
                      ...
                      <td key={cell.id} className={`p-0 ${specificError ? 'bg-red-900/30' : ''}`}>
                        {specificError ? (
                          <Tooltip>
                            ...
                            <TooltipContent className="bg-background border-red-500 space-y-2 p-2">
                              ...
                              {specificError.suggestion && (
                                <div className="pt-2 border-t border-border flex items-center gap-2">
                                  ...
                                  <Button variant="ghost" size="sm" className="h-auto p-1 text-yellow-400 hover:bg-yellow-900/50 hover:text-yellow-300"
                                    onClick={() => {
                                      ...
                                      dispatch({ type: 'UPDATE_CELL', payload: { entity, rowIndex: row.index, columnId: columnId, value: newValue.join(',') } });
                                    }}
                                  >
                                    {specificError.suggestion}
                                  </Button>
                              ...
                              {/* THIS IS THE NEWLY ADDED UI FEATURE */}
